		...
		for (const setting of settingsTables) {
			...
			if (setting.elements.length > 0) {
				for (const element of setting.elements) {
					await client.query(
						`INSERT INTO ${tableName} (value) VALUES ($1) ON CONFLICT DO NOTHING`,
						[element]
					);
				}
			}
...
async function getSchema(req) {
	...
	try {
		const result = await client.query(
			"SELECT key FROM login WHERE key = $1",
			[key]
		);
...
app.post("/api/register", async (req, res) => {
	...
	try {
		await client.query("INSERT INTO login (name, key) VALUES ($1, $2)", [
			name,
			key
		]);
...
app.get("/api/group/:key", async (req, res) => {
	...
	try {
		const result = await client.query(
			"SELECT name FROM login WHERE key = $1",
			[key]
		);
...
app.put("/api/participants/:id", async (req, res) => {
	try {
		...
		await client.query(
			`UPDATE ${schema}.participants SET name = $1, avatar = $2 WHERE id = $3`,
			[name, avatar, userId]
		);
...
app.delete("/api/participants/:id", async (req, res) => {
	try {
		...
		await client.query(`DELETE FROM ${schema}.participants WHERE id = $1`, [
			userId
		]);
...
app.post("/api/cars", async (req, res) => {
	try {
		const schema = await getSchema(req);
		const { name, driver, passengers, seats, model, brand, color } =
			req.body;
...
app.put("/api/cars/:id", async (req, res) => {
	try {
		...
		await client.query(
            ...
            color = $7
            WHERE id = $8`,
			[name, driver, passengers, seats, model, brand, color, carId]
		);
...
app.post("/api/things", async (req, res) => {
	try {
		...
		await client.query(
			`INSERT INTO ${schema}.things (name, category, per_person, unit_id, weight) VALUES ($1, $2, $3, $4, $5)`,
			[name, category, perPerson, unit, weight]
		);
...
app.delete("/api/things/:id", async (req, res) => {
	try {
		...
		// Führe die DELETE-Abfrage in der entsprechenden Tabelle des ermittelten Schemas aus
		await client.query(`DELETE FROM ${schema}.things WHERE id = $1`, [
			thingID
		]);
...
app.post("/api/tours", async (req, res) => {
	try {
		const schema = await getSchema(req);
		const { tourData, tourMembers, tourThings, tourCars, tourAssignments } =
			req.body;

		await client.query(
			`INSERT INTO ${schema}.tours (tour_data, tour_members, tour_things, tour_cars, tour_assignments) VALUES ($1, $2, $3, $4, $5)`,
			[tourData, tourMembers, tourThings, tourCars, tourAssignments]
		);
...
app.get("/api/tour/:id", async (req, res) => {
	try {
		...
		const tourResult = await client.query(
			`SELECT * FROM ${schema}.tours WHERE id = $1`,
			[tourId]
		);
		...
		// Zusatzdaten abrufen
		const [thingsDataById, membersDataById, carsDataById] =
			await Promise.all([
		...
		const totalWeight = Object.values(thingsDataById).reduce(
			(sum, thing) => sum + (thing.weight || 0),
			0
		);
...
app.get("/api/tours", async (req, res) => {
	try {
		...
		const toursResponse = tours.map((tour) => {
			...
			const totalWeight = tourThingsIds.reduce(
				(sum, id) => sum + (thingsDataById[id]?.weight || 0),
				0
			);
...
app.delete("/api/tours/:id", async (req, res) => {
	try {
		...
		// Führe die DELETE-Abfrage in der entsprechenden Tabelle des ermittelten Schemas aus
		await client.query(`DELETE FROM ${schema}.tours WHERE id = $1`, [
			tourID
		]);
...
app.get("/api/settings/:table", async (req, res) => {
	try {
		...
		const result = await client.query(
			`SELECT * FROM ${schema}.${table.toLowerCase()}`
		);
...
app.post("/api/settings/:table", async (req, res) => {
	try {
		...
		await client.query(
			`INSERT INTO ${schema}.${table.toLowerCase()} (value) VALUES ($1)`,
			[value]
		);
...
app.put("/api/settings/:table/:id", async (req, res) => {
	try {
		...
		await client.query(
			`UPDATE ${schema}.${table.toLowerCase()} SET value = $1 WHERE id = $2`,
			[value, id]
		);
...
app.delete("/api/settings/:table/:id", async (req, res) => {
	try {
		...
		await client.query(
			`DELETE FROM ${schema}.${table.toLowerCase()} WHERE id = $1`,
			[id]
		);
